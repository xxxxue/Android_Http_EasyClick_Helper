...
class EcHelper {
    companion object {
        ...
        private const val EC_URL = "http://127.0.0.1:20390/global"
        ...
        fun setSaveLog(): String {
            ...
            return getDataStringInJson(result)
        ...
        fun clickPoint(x: Int, y: Int): Boolean {
            ...
            return getDataBoolInJson(result)
        }
...
//            }
 ...
 var touch2 = [
    ...
    var p = {
        ...
        private fun getDataInJson(content: String): JsonElement {
            val jsonObject = JsonParser.parseString(content).asJsonObject
            return jsonObject.get("data")
        }

        private fun getDataBoolInJson(content: String): Boolean {
            val jsonObject = JsonParser.parseString(content).asJsonObject
            return jsonObject.get("data").asBoolean
        }

        private fun getDataStringInJson(content: String): String {
            val jsonObject = JsonParser.parseString(content).asJsonObject
            return jsonObject.get("data").asString
        }

        private fun getDataStringInJsonByTypeTemplate(typeStr: String): String {
            val res = typeTemplate(typeStr)
            return getDataInJson(res).asString
        }

        private fun getDataBoolInJsonByMsgTemplate(typeStr: String, msg: String): Boolean {
            val res = msgTemplate(typeStr, msg)
            return getDataInJson(res).asBoolean
        }

        private fun getDataBoolInJsonByTypeTemplate(typeStr: String): Boolean {
            val res = typeTemplate(typeStr)
            return getDataInJson(res).asBoolean
        }

        private fun typeTemplate(typeStr: String): String {
            val jsonObject = JSONObject()
            jsonObject.put("type", typeStr)
            val result = post(jsonObject.toString())
            Log.d(TAG, "$typeStr: $result")
            return result
        }

        private fun msgTemplate(typeStr: String, msg: String): String {
            val jsonObject = JSONObject()
            jsonObject.put("type", typeStr)
        ...
        private fun <T> clickTemplate(nodeEnum: NodeEnum, data: T): Boolean {

            var res = getOneNodeInfo(nodeEnum, data)
            if (res.isSuccess) {
                if (res.data != null) {
                    var centerVisibleBounds = getCenterVisibleBounds(res.data!!)
